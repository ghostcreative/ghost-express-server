...
class ErrorHandlerSetup {
  ...
  static logErrorHandler (err, req, res, next) {
    ...
    const errorMessage = `GhostExpressServer ErrorHandlerSetup.logErrorHandler - ${err ? err.message : 'Unknown server error'}`;
 ...
 static  _formatErrorMessage (err, req, res, next) {
    ...
    if (err.isJoi) err.message = err.message.replace( /(^.*\[|\].*$)/g, '' );
    next(err);
  }
