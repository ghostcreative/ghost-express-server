...
const ValidateFunc = {
  ...
  execute: (req, res) => {
    ...
    .tap(_permissions_ => permissions = _permissions_)
    .then(() => {
    ...
    .catch(err => {
      if (_.has(req.routeOptions, 'options.requirePermissions') && req.routeOptions.options.requirePermissions === false) return permissions;
      else throw err;
    })
  },
