...
class GhostExpressServer {
  ...
  useRouter (pathOrMiddleware, middleware) {
    ...
    else if (middleware.register) this._server.use(pathOrMiddleware, this._registerGhostRouter(middleware));
  ...
  _setupTrailingMiddleware () {
    ...
    return Promise.resolve()
    .then(() => routesSetup.configure(this._serviceLocator.getConfig('routes'), this, this._serviceLocator.getLogger()))
    .then(() => errorSetup.configure(this._serviceLocator.getConfig('error'), this._server, this._serviceLocator.getLogger()))
  }
  ...
  _registerGhostRouter (router) {
    ...
    router.db = this._serviceLocator.getDbService().getDb();
