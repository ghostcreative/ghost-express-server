...
class GhostExpressServer {
  ...
  _setupTrailingMiddleware () {
    ...
    return Promise.all([
        routesSetup.configure(this._serviceLocator.getConfig('routes'), this._server, this._serviceLocator.getLogger()),
        errorSetup.configure(this._serviceLocator.getConfig('error'), this._server, this._serviceLocator.getLogger())
    ])
  }
